...
def process_bmp_files(input_dir, output_dir=None, suffix="_reversed"):
    ...
    print(f"\nFound {len(bmp_files)} BMP files in '{input_dir}'")
...
def main():
    ...
    parser.add_argument('-s', '--suffix', help='Suffix for output filenames (will be asked interactively if not provided)')
    ...
    print("\n" + "="*60)
    print("BMP Phase Map Reversal Tool")
    print("="*60)
    
    # Ask for naming suffix if not provided via command line
    if args.suffix is None:
        args.suffix = input("Enter suffix for output filenames (default: _reversed): ").strip()
        if not args.suffix:
            args.suffix = "_reversed"
    ...
    if args.preview:
        ...
        for bmp_file in bmp_files:
            preview_name = bmp_file.stem + args.suffix + bmp_file.suffix
        ...
        if args.output_dir:
            ...
            print(f"\nOutput directory: {args.input_dir} (same as input)")
        return
