...
# Normalize data
x_train = (x_train / np.max(np.abs(x_train))).astype(np.float16)
x_val = (x_val / np.max(np.abs(x_val))).astype(np.float16)
x_test = (x_test / np.max(np.abs(x_test))).astype(np.float16)
...
opt = tf.keras.optimizers.Adam(learning_rate=0.1, clipnorm=1.0)
...
train_dataset = tf.data.Dataset.from_tensor_slices((x_train, y_train)).shuffle(500).batch(16).map(lambda x, y: (tf.cast(x, tf.float16), tf.cast(y, tf.float16)))
end_time = time.time()
print(f"Data loading time: {end_time - start_time:.2f} seconds")
val_dataset = tf.data.Dataset.from_tensor_slices((x_val, y_val)).batch(16).map(lambda x, y: (tf.cast(x, tf.float16), tf.cast(y, tf.float16)))
