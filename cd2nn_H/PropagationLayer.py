...
class PropagationLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        h_real = h_real[:, :W + 1]
        h_imag = h_imag[:, :W + 1]
        print("h_real shape after slicing:", h_real.shape)
        print("h_imag shape after slicing:", h_imag.shape)
    ...
    def call(self, inputs):
        ...
        print("dtype tf.signal.rfft2d(re_u):", tf.signal.rfft2d(re_u).dtype)
        print("shape tf.signal.rfft2d(re_u):", tf.signal.rfft2d(re_u).shape)
        print("dtype self.h_real:", self.h_real.dtype)
        print("shape self.h_real:", self.h_real.shape)
        h_real_frequency = self.h_real[:, :self.shape_[0] + 1]
        h_imag_frequency = self.h_imag[:, :self.shape_[0] + 1]
        ...
        # tf.print("Power after convolutions:", power_after)
